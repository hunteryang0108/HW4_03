<x-layouts.app :title="$post->title">
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg p-6 mb-6 shadow-sm hover:shadow-md transition-shadow">
        <div class="flex justify-between items-start">
            ...
            <div class="flex space-x-2">
                ...
                class="text-accent hover:bg-accent/10 p-2 rounded-full transition-colors flex items-center">
                ...
                <form action="{{ route('posts.destroy', $post) }}" method="POST" class="inline" 
                    ...
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 p-2 rounded-full transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <form action="{{ route('posts.favorite', $post) }}" method="POST">
                    @csrf
                    <button type="submit" class="flex items-center {{ auth()->user() && auth()->user()->hasFavorited($post) ? 'text-yellow-500' : 'text-zinc-500' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" fill="{{ auth()->user() && auth()->user()->hasFavorited($post) ? 'currentColor' : 'none' }}" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                        </svg>
                        <span>{{ auth()->user() && auth()->user()->hasFavorited($post) ? '已收藏' : '收藏' }}</span>
                    </button>
                </form>
